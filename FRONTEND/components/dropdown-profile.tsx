...
//TODO signout cancella cookie, maybe set auth false
export default function DropdownProfile({ align, username, avatar  }: {
  align?: 'left' | 'right',
  username?: string,
  avatar?: string
}) {
  return (
    <Menu as="div" className="relative inline-flex">
      <Menu.Button className="inline-flex justify-center items-center group">
        <Image className="w-8 h-8 rounded-full" src={avatar&&avatar!="fromAPI"?avatar:UserAvatar} width={32} height={32} alt="ProfilePic" />
      ...
      >
        {/* <div className="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200 dark:border-slate-700">
          <div className="font-medium text-slate-800 dark:text-slate-100">{username? username : 'Guest'}</div>
        </div> */}
        <Menu.Items as="ul" className="focus:outline-none">
          <Menu.Item as="li">
            {({ active }) => (
              <Link className={`font-medium text-sm flex items-center py-1 px-3 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-indigo-500'}`} href="/community/profile">
                Profile
              </Link>
            )}
          </Menu.Item>
